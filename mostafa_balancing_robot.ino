...
int acc_calibration_value = 650;                            //Enter the accelerometer calibration value

//Various settings
float pid_p_gain = 2.2;                                       //Gain setting for the P-controller (15)
float pid_i_gain = 0.4;                                      //Gain setting for the I-controller (1.5)
float pid_d_gain = 0.1;                                       //Gain setting for the D-controller (30)
float turning_speed = 20;                                    //Turning speed (20)
float max_target_speed = 20;                                //Max target speed (100)
...
void setup(){
  ...
  pinMode(7, OUTPUT);                                                       //Configure digital poort 5 as output
...
ISR(TIMER2_COMPA_vect){
  ...
  if(throttle_counter_left_motor > throttle_left_motor_memory){             //If the number of loops is larger then the throttle_left_motor_memory variable
    ...
    if(throttle_left_motor_memory < 0){                                     //If the throttle_left_motor_memory is negative
      PORTD &= 0b11011111;                                                  //Set output 3/5 low to reverse the direction of the stepper controller
      throttle_left_motor_memory *= -1;                                     //Invert the throttle_left_motor_memory variable
    }
    else PORTD |= 0b00100000;                                               //Set output 3/5 high for a forward direction of the stepper motor
  }
  else if(throttle_counter_left_motor == 1)PORTD |= 0b00010000;             //Set output 2/4 high to create a pulse for the stepper controller
  else if(throttle_counter_left_motor == 2)PORTD &= 0b11101111;             //Set output 2/4 low because the pulse only has to last for 20us 
  ...
  if(throttle_counter_right_motor > throttle_right_motor_memory){           //If the number of loops is larger then the throttle_right_motor_memory variable
    ...
    if(throttle_right_motor_memory < 0){                                    //If the throttle_right_motor_memory is negative
      PORTD |= 0b10000000;                                                  //Set output 5/7 low to reverse the direction of the stepper controller
      throttle_right_motor_memory *= -1;                                    //Invert the throttle_right_motor_memory variable
    }
    else PORTD &= 0b01111111;                                               //Set output 5/7 high for a forward direction of the stepper motor
  }
  else if(throttle_counter_right_motor == 1)PORTD |= 0b01000000;            //Set output 4/6 high to create a pulse for the stepper controller
  else if(throttle_counter_right_motor == 2)PORTD &= 0b10111111;            //Set output 4/6 low because the pulse only has to last for 20us
}
